...
- `sw.js` — service worker. `const CACHE = 'minder-v19'`. **Network-first** voor de app-pagina (verse versie online, val terug op cache offline), **cache-first** voor iconen, en **cross-origin/PSD2-backend wordt nooit gecachet** (altijd live).
...
- **Bij elke release die de cache moet verversen: hoog `CACHE` in `sw.js` op** (`minder-v19` → `minder-v20`, …). De oude cache wordt in `activate` opgeruimd.
