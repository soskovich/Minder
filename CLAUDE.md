...
- `sw.js` — service worker. `const CACHE = 'minder-v104'` (het actuele nummer staat altijd in `sw.js` zelf). **Network-first** voor de app-pagina (verse versie online, val terug op cache offline), **cache-first** voor iconen, en **cross-origin/PSD2-backend wordt nooit gecachet** (altijd live).
...
- **Bij elke release die de cache moet verversen: hoog `CACHE` in `sw.js` op** (`minder-v104` → `minder-v105`, …). De oude cache wordt in `activate` opgeruimd.
