...
- `sw.js` — service worker. `const CACHE = 'minder-v114'` (het actuele nummer staat altijd in `sw.js` zelf). **Network-first** voor de app-pagina (verse versie online, val terug op cache offline), **cache-first** voor iconen, en **cross-origin/PSD2-backend wordt nooit gecachet** (altijd live).
...
- **Bij elke release die de cache moet verversen: hoog `CACHE` in `sw.js` op** (`minder-v114` → `minder-v115`, …). De oude cache wordt in `activate` opgeruimd.
...
- **Drie drill-downs achter de budgetkaart, elk met een eigen vraag** (`v114`): de **ring en titel** openen `openBudgetCompare()` (hoe doe ik het: besteed vs. budget, tempo), **Uitgegeven** opent `openMonthSpend()` (waar ging mijn geld heen) en **Maandbudget** opent `openPotjesVerdeling()` (hoe is mijn plan verdeeld, nu en vanaf volgende maand). Die laatste toont bewust géén besteding per potje: dat is de eerste, en twee sheets die hetzelfde zeggen is er één te veel. De som van de rijen is per constructie het getal in de kop — sectie 1 uit `totalBudget()`, sectie 2 uit `plannedBudgets()`/`plannedTotalBudget()`, nooit een eigen som (`v104`/`v111`). Bewerken blijft `openPotje()` per rij en `setBudget()` via de voetlink: een drill-down is een extra ingang, nooit een tweede editor (`v61`). Voor een afgeronde maand vervalt sectie 2 en staat er dat je potjes een instelling van nu zijn — er is geen potjes-historie per maand, dus die suggestie wekken we niet (`v59`/`v73`).
