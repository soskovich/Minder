...
- `sw.js` — service worker. `const CACHE = 'minder-v23'`. **Network-first** voor de app-pagina (verse versie online, val terug op cache offline), **cache-first** voor iconen, en **cross-origin/PSD2-backend wordt nooit gecachet** (altijd live).
...
- **Bij elke release die de cache moet verversen: hoog `CACHE` in `sw.js` op** (`minder-v23` → `minder-v24`, …). De oude cache wordt in `activate` opgeruimd.
